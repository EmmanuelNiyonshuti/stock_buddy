...
def get_all_businesses(db_session, user_id, page, per_page):
    associated_business = db_session.query(user_business_association).filter_by(user_id=user_id).all()
    business_ids = [assoc.business_id for assoc in associated_business]
